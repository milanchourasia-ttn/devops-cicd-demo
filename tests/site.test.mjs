...
test("website contains the required deployment message", async () => {
  ...
  assert.match(html, /id="version">\d+\.\d+</);
});
...
test("local assets referenced by the page exist", async () => {
  ...
  assert.ok(javascript.length > 50);
});

test("page includes responsive and accessibility metadata", async () => {
  const html = await readProjectFile("index.html");

  assert.match(html, /name="viewport"/);
  assert.match(html, /<html lang="en">/);
  assert.match(html, /aria-label="Deployment pipeline"/);
});
